 ...
 * */
{
  ...
  if (t->sp.typicalXcase)
	  for (i = 0; i < N; ++i)
		  t->rgxmean[i] += t->sigma * t->rgD[i] * Global::msp_global->mp_normalPro->Next();//cmaes_random_Gauss(&t->rand);
...
cmaes_SamplePopulation(cmaes_t *t)
{
  ...
  for (iNk = 0; iNk < t->sp.lambda; ++iNk)
    { /* generate scaled cmaes_random vector (D * z)    */
		for (i = 0; i < N; ++i)
			if (flgdiag)
				t->rgrgx[iNk][i] = xmean[i] + t->sigma * t->rgD[i] * Global::msp_global->mp_normalPro->Next();// cmaes_random_Gauss(&t->rand);
			else
				t->rgdTmp[i] = t->rgD[i] * Global::msp_global->mp_normalPro->Next();// cmaes_random_Gauss(&t->rand);
      if (!flgdiag)
